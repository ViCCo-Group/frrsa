...
def tune_rsa(output, inputs, outer_k=5, outer_reps=10, splitter='random', \
             hyperparams=None, score_type='pearsonr', sanity=False, rng_state=None):
...
#%%    
    predicted_RDM, predictions, scores, betas \
        = start_cross_validation(splitter, outer_k, outer_reps, rng_state, \
                                 n_conditions, output, inputs_z, score_type, \
                                 predicted_RDM, predicted_RDM_counter, \
                                 hyperparams, n_outputs)
            
    return predicted_RDM, predictions, unfitted_scores, scores, betas


#%%
def start_cross_validation(splitter, outer_k, outer_reps, rng_state, \
                           n_conditions, output, inputs_z, score_type,\
                           predicted_RDM, predicted_RDM_counter, \
                           hyperparams, n_outputs):
    ...
    predicted_RDM = collapse_RDM(n_conditions, n_outputs, predicted_RDM, predicted_RDM_counter)

    return predicted_RDM, predictions, scores, betas
...
def collapse_RDM(n_conditions, n_outputs, predicted_RDM, predicted_RDM_counter):
    ...
    predicted_RDM[idx_low] = average_preds
    for var in range(n_outputs):
        predicted_RDM[:,:,var] = predicted_RDM[:,:,var] + predicted_RDM[:,:,var].T
